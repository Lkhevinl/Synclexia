...
describe('Test Case CASE-077 (Validate opening FAQ section)', () => {
  ...
  test('Open FAQ section - able to view FAQ', () => {
    ...
    if (result.success && result.faqLoaded && result.data) {
    ...
    expect(result.faqLoaded).toBe(true);
  ...
  test('FAQ includes all categories - able to view FAQ', () => {
    const expectedResult = 'Able to view FAQ';
    const result = viewFAQ();
  ...
  test('FAQ questions have Q&A format - able to view FAQ', () => {
    const result = viewFAQ();
    const firstCategory = result.categories[0];
    const firstQuestion = firstCategory.items[0];

    console.log('Test Case ID: CASE-077');
    console.log('Test Case Description: Validate opening FAQ section');
    console.log(`Category: ${firstCategory.name}`);
    console.log(`Question: ${firstQuestion.question}`);
    console.log(`Answer: ${firstQuestion.answer.substring(0, 50)}...`);

    if (firstQuestion.question && firstQuestion.answer && firstQuestion.answer.length > 0) {
      console.log('Outcome: PASSED - Q&A format verified');
    } else {
      console.log('Outcome: FAILED');
    }
  ...
  test('FAQ data includes last updated date - able to view FAQ', () => {
    const result = viewFAQ();
